---
title: "24133064_NguyenThiKieuTrang_Bai_tap_lab2"
...
### Truy xuất tên sinh viên bằng 3 cách: dùng $, dùng [[ ]] với tên thành phần, và dùng [[ ]] với chỉ số vị trí.
...
### Trích thành phần scores bằng student_lst["scores"] và student_lst[["scores"]]. Dùng class() để giải thích sự khác biệt giữa hai kết quả
...
### Giải thích: student_lst["scores] trả về một list con; student_lst[["scores"]] trả về giá trị bên trong phần tử scores

## e. Thêm và xóa thành phần trong list

### Tính điểm trung bình của sinh viên và thêm vào list với tên mean_score. Sau đó xóa thành phần passed khỏi list.
...
## a. Tạo list điểm; Tạo list score_lst gồm 4 thành phần tương ứng với 4 môn học ở trên.
...
## c. Tìm môn có điểm trung bình cao nhất Dựa trên kết quả ở câu b, viết code để trả về tên môn có điểm trung bình cao nhất
...
### Dùng unlist() để chuyển score_lst thành một vector. Tính điểm trung bình chung của toàn bộ các điểm, bỏ qua NA.
...
### Thêm biến total_score, tính theo công thức: total_score = 0.4 × midterm + 0.6 × final. Sau đó thêm biến result, nhận giá trị "Pass" nếu total_score >= 5.5, ngược lại là "Fail".
...
### Sắp xếp students_df theo total_score giảm dần. Sau đó tạo biến rank thể hiện thứ hạng của sinh viên theo điểm tổng.
...
### Tạo một data frame new_students_df gồm 2 sinh viên mới như bảng dưới đây. Kết hợp new_students_df vào students_df để tạo all_students_df. Sau đó tính lại total_score, result và rank cho toàn bộ dữ liệu.id name gender class attendance midterm final total_score result rank city 109 Nam M C 92 8.1 8.6 NA NA NA HN 110 Oanh F A 78 7.4 7.9 NA NA NA HCM
...
### Dùng sapply() để kiểm tra class của từng cột trong data frame sau khi merge. Sau đó dùng sapply() để tính số lượng giá trị thiếu trong từng cột.
...
### Tạo program_factor từ program. Tạo satisfaction_factor là ordered factor với thứ tự level: Low < Medium < High.
...
### Tạo biến grade_level dựa trên total_score theo quy tắc dưới đây. Sau đó dùng table() để đếm số sinh viên theo grade_level. Khoảng điểm total_score Nhãn grade_level
